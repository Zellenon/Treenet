...
def get_precision2(prediction: TPredict, targets: TTarget, mlb: TMlb = None, classes: TClass = None, top=5, ave='binary'):
    if ave == 'binary':
...
get_p_1 = partial(get_precision2, top=1)
get_p_3 = partial(get_precision2, top=3)
get_p_5 = partial(get_precision2, top=5)
get_p_7 = partial(get_precision2, top=7)
get_p_9 = partial(get_precision2, top=9)
get_p_11 = partial(get_precision2, top=11)
get_p_10 = partial(get_precision2, top=10)

def get_recall2(prediction: TPredict, targets: TTarget, mlb: TMlb = None, classes: TClass = None, top=5, ave='binary'):
    # if ave == 'binary':
    #     return get_precision(prediction, targets, mlb, classes, top)
    mlb = get_mlb(classes, mlb, targets)
    if not isinstance(targets, csr_matrix):
...
def get_f1(prediction: TPredict, targets: TTarget, mlb: TMlb = None, classes: TClass = None, top=5, ave='binary'):
    # if ave == 'binary':
    #     return get_precision(prediction, targets, mlb, classes, top)
    mlb = get_mlb(classes, mlb, targets)
    if not isinstance(targets, csr_matrix):
