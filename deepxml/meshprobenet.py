...
from deepxml.treenet import TreeNet
...
class Probe(nn.Module):
    ...
    def forward(self, birnn_outputs, masks):
        attn = (
            self.self_attn(birnn_outputs).transpose(1, 2).masked_fill(~masks, -np.inf)
        )  # (batch, n_probes, in_len)
...
class MeSHProbeNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.meshprobes = MeSHProbes(
            hidden_size * 2, n_probes
        )  # *2 because of bidirection
        self.plaincls = PlainC(labels_num, hidden_size * 2, n_probes)

    def forward(self, input_variables):
        emb_out, lengths, masks = self.emb(input_variables)
        birnn_outputs = self.lstm(emb_out, lengths.cpu())  # N, L, hidden_size * 2
...
class CorNetMeSHProbeNet(nn.Module):
    def __init__(
        self, emb_size, hidden_size, n_layers, n_probes, labels_num, dropout, **kwargs
    ):
        super(CorNetMeSHProbeNet, self).__init__()
        self.meshprobenet = MeSHProbeNet(
            emb_size, hidden_size, n_layers, n_probes, labels_num, dropout, **kwargs
        )
...
class TreeNetMeSHProbeNet(nn.Module):
    def __init__(
        self, emb_size, hidden_size, n_layers, n_probes, labels_num, dropout, **kwargs
    ):
        super(TreeNetMeSHProbeNet, self).__init__()
        self.meshprobenet = MeSHProbeNet(
            emb_size, hidden_size, n_layers, n_probes, labels_num, dropout, **kwargs
        )
        self.treenet = TreeNet(labels_num, **kwargs)

    def forward(self, input_variables):
        raw_logits = self.meshprobenet(input_variables)
        tree_logits = self.treenet(raw_logits)
        return tree_logits
