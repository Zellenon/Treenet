...
def evaluate(cfg: DatasetConfig, model_cfg):
    ...
    test_x, _ = get_data(cfg.data['test'].text)
    logger.info(F'Size of Test Set: {len(test_x)}')
    trained_model_path = Path(format("results/models/{model_cfg[\"name\"]}-{cfg.name}"))

    logger.info('Predicting')
    test_loader = DataLoader(MultiLabelDataset(test_x), model_cfg['batch_size'], num_workers=4)
    model = GPipeModel(model_cfg['name'], labels_num=labels_num, model_path=trained_model_path, emb_init=cfg.emb_init, 
        **cfg.model, **model_cfg['model']) if 'gpipe' not in model_cfg else Model(network=model_dict[model_cfg['name']], labels_num=labels_num, model_path=trained_model_path, emb_init=cfg.emb_init,
            **cfg.model, **model_cfg['model'])
    scores, labels = model.predict(test_loader, k=model_cfg['predict'].get('k', 100))
    ...
    targets = np.array([[1 if w in row else 0 for w in alllabels] for row in target_labels])


    print("Loading labels")
    predicted_labels = np.load(labels, allow_pickle=True)
    predicted_scores = np.load(scores, allow_pickle=True)
    temp = [zip(predicted_labels[i], predicted_scores[i]) for i in range(len(predicted_scores))]
    ...
    score_funcs = [(metrics.precision_score, "Precision"), (metrics.recall_score, "Recall"), (metrics.f1_score, "F1")]
    ...
    for threshhold in threshholds:
        ...
        for average in averages:
            for func, name in score_funcs:
                print(f"{average}-{name} at p>{threshhold}: {func(targets,temp_predictions, average=average)}")
    for top_n in n:
        ...
        for average in averages:
            for func, name in score_funcs:
                print(f"{average}-{name} at n={top_n}: {func(targets,temp_predictions, average=average)}")
